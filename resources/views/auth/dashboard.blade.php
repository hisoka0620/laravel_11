...
<h2 class="text-xl sm:text-2xl font-semibold mb-6 text-center sm:text-left">📝 Your To-Do List</h2>
...
<div
    ...
    <p class="text-blue-500 text-center sm:text-left">
        You currently have
        <span class="font-bold text-blue-700">{{ $unfinishedCount }}</span> unfinished task{{ $unfinishedCount > 1 ? 's'
        : ''}}.
    </p>
</div>
@endif
    ...
    <div class="flex flex-col sm:flex-row items-center sm:items-end justify-between w-full sm:flex-grow">
        ...
        <div class="text-center sm:text-left">
            <p class="sm:text-base">
                You currently have <strong>{{ $expiredCount }}</strong> overdue {{ Str::plural('task', $expiredCount) }}.
            </p>
...
<div
    class="flex flex-col sm:flex-row sm:items-center text-yellow-600 bg-yellow-100 border-l-4 border-yellow-500 p-4 rounded mb-6 shadow-sm animate-fade-in">
    ...
    <p class="text-center sm:text-left flex-shrink-0">
        You currently have <span class="font-bold">{{ $urgentTasksCount }}</span> task{{ $urgentTasksCount > 1 ? 's' : '' }} that are due soon!
    </p>
...
<div class="flex justify-center text-center">
    <a href="{{ route('tasks.create') }}"
        class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition w-full sm:w-2/5">
        Let's go to create a task!
    </a>
</div>
@else
    ...
    <div x-data="taskCard(@js($task))" x-init="init()" x-cloak
        ...
        <div>
            <template x-if="showUrgent">
                <div class="border-l-4 border-yellow-500 bg-yellow-100 p-2 mb-2 rounded">
                    <h3 class="text-yellow-600 font-bold animate-pulse">🔥 Tasks due soon</h3>
                </div>
            </template>
